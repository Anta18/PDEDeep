...
class BSDESolver(object):
    def __init__(self, config, bsde, log_dir):
        ...
        self.optimizer = tf.keras.optimizers.Adam(
            learning_rate=lr_schedule, epsilon=1e-8, clipnorm=1.0
        )
        self.checkpoint = tf.train.Checkpoint(
            optimizer=self.optimizer, model=self.model
        )
        self.checkpoint_manager = tf.train.CheckpointManager(
            self.checkpoint, directory=self.log_dir, max_to_keep=5
        )
...
class NonsharedModel(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        all_one_vec = tf.ones(shape=[tf.shape(dw)[0], 1], dtype=self.net_config.dtype)
...
class FeedForwardSubNet(tf.keras.Model):
    def __init__(self, config):
        ...
        self.dense_layers = [
            tf.keras.layers.Dense(num_hiddens[i], use_bias=False)
            for i in range(len(num_hiddens))
        ]
        self.output_layer = tf.keras.layers.Dense(dim)
